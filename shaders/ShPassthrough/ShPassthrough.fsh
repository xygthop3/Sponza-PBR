...
PixelShaderOutput main(PixelShaderInput INPUT) {
  ...
  //Direct diffuse
  float3 diffuse = kD * albedo.rgb  * radiance * costhetaL;
  
  //Direct fake "grabage" hemispheric ambient
  float up = PN.z * 0.5 + 0.5;
  float3 ambient = float3(0.1, 0.1, 0.1) + up * float3(0.3, 0.3, 0.3);
  ambient = ambient * albedo.rgb;
  
  float3 directLighting = ambient + diffuse + specular;

  //IBL 
  float3 indirectDiffuseLighting = texCubemap.SampleLevel(sampCubemap, cube2uv(N), 6) * diffuse;
  
  float mipIndex = roughness * 6;
  float3 reflectedColor = texCubemap.SampleLevel(sampCubemap, cube2uv(R), mipIndex);
  
  float3 environmentBRDF = texBRDFLUT.Sample(sampBRDFLUT, max(float2(roughness, costhetaV), 0.0001));
  float3 specularAlbedo = albedo.rgb * metalness;
  float3 indirectSpecular = reflectedColor * (specularAlbedo * environmentBRDF.x + environmentBRDF.y);
  float3 indirectLighting = indirectDiffuseLighting + indirectSpecular;
  
  float3 color = directLighting + indirectLighting;
  
  OUTPUT.Color0 = float4(uchimura(color.r), uchimura(color.g), uchimura(color.b), 1.0); //Stop being lazy and FIX THIS so tonemapping is done on a final surface
